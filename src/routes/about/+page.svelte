  ...
  <div class="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 pt-32 pb-20">
    ...
    <div class="flex flex-col lg:flex-row gap-16 lg:gap-24 items-start">
        ...
        <div class="w-full lg:w-[35%] mt-10 lg:mt-0 transform transition-all duration-700 delay-300 {loaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'}">
            ...
            <div 
                class="relative w-full aspect-[3/4] cursor-pointer group select-none"
                ...
                {#each [...photos].reverse() as photo, i}
                    ...
                    >
                        <div class="w-full h-full overflow-hidden relative rounded-xl grayscale-[20%] contrast-[110%] sepia-[10%] transition-all duration-500 group-hover:grayscale-0">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-tr from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                        </div>
                    </div>
                {/each}

                <!-- Hint -->
                <div class="absolute -bottom-12 left-1/2 -translate-x-1/2 text-sm text-zinc-400 dark:text-zinc-600 animate-pulse whitespace-nowrap">
                   (Click to shuffle)
                </div>
            </div>
    ...
    <!-- Music Section -->
    <div class="mt-24 md:mt-32 transform transition-all duration-700 delay-500 {loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}">
        <h2 class="font-clash text-3xl font-semibold mb-10">My on repeat songs</h2>
        ...
        {:else}
             ...
             <div class="p-6 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl text-center">
                <p class="text-zinc-500">Connect to Spotify API to see tracks.</p>
             </div>
        {/if}

        <p class="mt-6 text-sm text-zinc-500 dark:text-[#A9A9BD]">
            *Syncs automatically with my Spotify Top Tracks.
        </p>
    </div>
  </div>
